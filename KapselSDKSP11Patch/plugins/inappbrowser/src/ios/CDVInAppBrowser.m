...
{
    if (imageView == NULL) {
        self.inAppBrowserViewController.view.window.hidden = NO;
    } else {
        [imageView removeFromSuperview];
        
        // in case of that if subviews on viewController has existed, then remove the image view that is used for hiding screenshot.
        // (for the case after first tip screen on FC)
        for(UIImageView *subview in [self.viewController.view subviews]) {
...
{
    // use inAppBrowserViewController for hiding sceenshot on inappbrowser
    NSString *imgName = [self getImageName:self.inAppBrowserViewController.interfaceOrientation delegate:(id<CDVScreenOrientationDelegate>)self.inAppBrowserViewController device:[self getCurrentDevice]];
    
    UIImage *splash = [UIImage imageNamed:imgName];
    if (splash == NULL) {
        self.inAppBrowserViewController.view.window.hidden = YES;
    } else {
        imageView = [[UIImageView alloc]initWithFrame:[self.inAppBrowserViewController.view bounds]];
        [imageView setImage:splash];
        [self.inAppBrowserViewController.view addSubview:imageView];
    }    
}


// for now, use privay plugin's private methods (getCurrentDevice(), getImageName()) to hide screenshot on inappbrowser for appswitcher
...
- (void)createViews
{
    ...
    if (_browserOptions.hidenavigation){
        if(_browserOptions.showshareoption){
        ...
        if(_browserOptions.showshareoption){
            [self.toolbar setItems:@[self.closeButton, flexibleSpaceButton,  self.shareButton, flexibleSpaceButton,  self.backButton, fixedSpaceButton, self.forwardButton]];  //original line
        }else{
...
- (void)shareAction {
    ...
    UIActivityViewController *activityVC = [[UIActivityViewController alloc] initWithActivityItems:@[self.webView.viewPrintFormatter] applicationActivities:nil];

    // exclude everything except printing
    activityVC.excludedActivityTypes = @[UIActivityTypePostToFacebook, UIActivityTypePostToTwitter, UIActivityTypePostToWeibo, UIActivityTypeMessage,UIActivityTypeMail, UIActivityTypeCopyToPasteboard, UIActivityTypeAssignToContact, UIActivityTypeSaveToCameraRoll, UIActivityTypeAddToReadingList, UIActivityTypePostToFlickr, UIActivityTypePostToVimeo, UIActivityTypePostToTencentWeibo, UIActivityTypeAirDrop];
